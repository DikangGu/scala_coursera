...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val list1 = w.groupBy((elem: Char) => elem.toLower).toList
    val res = list1.map((elem: (Char, String)) => (elem._1, elem._2.length()))
    res.sortBy(_._1)
  }
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case Nil => Nil
    case head :: tail => {
	  val list1 = for (w <- s) yield wordOccurrences(w)
	  val list2 = list1.flatten.groupBy((elem) => elem._1).toList
	  list2.map(elem => (elem._1, (for (o <- elem._2) yield o._2).sum)).sortBy(_._1)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
     def subMatch(cx: Char): List[Int] = for ((cy, ny) <- y) yield if (cy == cx) ny else 0
     def getValue(list: List[Int]): Int = {
       list.sorted(Ordering[Int].reverse)(0)
     }
    (for ((cx, nx) <- x) yield (cx, nx - getValue(subMatch(cx)))).filter(x => x._2 != 0)
  }
