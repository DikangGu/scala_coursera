...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    val larger = if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
    if (elem.retweets > larger.retweets) elem else larger
  }
  
  def descendingByRetweet: TweetList = {
    val highest = this.mostRetweeted
    val other = this.remove(highest).descendingByRetweet
    new Cons(highest, other)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter((elem: Tweet)=>{
      google.exists(x => elem.text.contains(x))
    })
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter((elem: Tweet)=>{
      apple.exists(x => elem.text.contains(x))
    })
  }
